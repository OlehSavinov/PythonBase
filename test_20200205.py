...
#     c = '0'*9 + str(number) + '0'*9
#     l = [c[i-int(c[i]):i] + c[i+1:i+1+int(c[i])] for i in range(len(c))]
#     s = [sum(map(int, l[i])) for i in range(len(l))]
#     c, s = c[9:-9], s[9:-9]
#     res =  [(c[i], s[i]) for i in range(len(s))]
#     m = min([i[1] for i in res])
#     for i in res:
#         if i[0] == '1' and i[1] == m:
#             return True
#     return False
